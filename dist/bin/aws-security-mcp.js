...
var LogAuditScanner = class {
  moduleName = "log_audit";
  async scan(ctx) {
    const { region, partition, accountId } = ctx;
    const startMs = Date.now();
    const findings = [];
    ...
    let resourcesScanned = 0;
    try {
      const ec2 = createClient(EC2Client7, region, ctx.credentials);
      const vpcIds = [];
      let vpcToken;
      do {
    ...
    try {
      const s3 = createClient(S3Client5, region, ctx.credentials);
      const bucketsResp = await s3.send(new ListBucketsCommand4({}));
    ...
    try {
      const elbv2 = createClient(ElasticLoadBalancingV2Client2, region, ctx.credentials);
      const lbResp = await elbv2.send(new DescribeLoadBalancersCommand2({}));
    ...
    try {
      const ct = createClient(CloudTrailClient2, region, ctx.credentials);
      const trailsResp = await ct.send(new DescribeTrailsCommand2({}));
    ...
    try {
      const rds = createClient(RDSClient4, region, ctx.credentials);
      let marker;
      const dbInstances = [];
      do {
        const resp = await rds.send(new DescribeDBInstancesCommand4({ Marker: marker }));
      ...
      for (const db of dbInstances) {
        ...
        if (db.engine.includes("mysql") || db.engine.includes("mariadb")) {
          hasRelevantLogs = db.exports.some(
            (e) => ["audit", "general", "slowquery"].includes(e)
          );
...
var MODULE_DESCRIPTIONS = {
  ...
  log_audit: "Checks whether key AWS logging services are enabled \u2014 VPC Flow Logs, S3 access logging, ELB access logs, CloudTrail multi-region, RDS audit logs, and CloudWatch Log Group retention."
};
